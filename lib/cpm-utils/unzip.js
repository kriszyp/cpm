if(typeof Packages !== "undefined"){
	exports.Unzip = function(fileContents){
		var zipStream = new Packages.java.util.zip.ZipInputStream(fileContents);
		return {
			readEntries: function(){
				...
				while((entry = zipStream.getNextEntry())){
					...
					do{
						var nextBuffer = Packages.java.lang.reflect.Array.newInstance(java.lang.Byte.TYPE, size + bufferSize);
						var bytesRead = 0;
						do{
							var segmentRead = zipStream.read(nextBuffer, bytesRead, nextBuffer.length - bytesRead);
							if(segmentRead > 0){
								bytesRead += segmentRead; 
							}
						}while(segmentRead > 0 && nextBuffer.length - bytesRead > 0);
						if(bytesRead > 0){
					...
					this.entries.push({
						compressionMethod: 0,
						fileName: String(entry.getName()),
						data: String(new java.lang.String(buffer))
					});
				}
			},
			isZipFile: function(){
