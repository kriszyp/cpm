...
function writeCurrentPackage(){
	fs.write(targetPackage + "/package.json", JSON.stringify(packageData, null, 4));
...
var requireConfig = {}, 
	...
	targetLib = targetPackage + '/' + (packageData.directories && packageData.directories.lib || '.'); 
//readPackagesConfig();
...
function installPackage(url, version, location, orNewer){
			...
			if(version && version != "current" && !orNewer){
				...
				for(var newVersion in versions){
					...
					if(versions[newVersion].dist &&
							(scoreVersion(newVersion) >= scoreVersion(bestVersion)) && (!version || compatible(version, newVersion) == -1)){
	...
	when(url, function(url){
		...
		var downloadPackagePath = targetPackage + "/" + location+ '/';
...
/*				print("needsUpgrade(url) " + needsUpgrade(url) + " " + downloadPackagePath + " " + targetPackage + "/packages/" + packageName);
	...
	function needsUpgrade(url, verbose){
		...
		if(existing){
			...
			try{
				var exists = fs.statSync(targetPackage + "/" + location).mtime.getTime();
			}catch(e){
			}
			if(exists){
				if(!version){
