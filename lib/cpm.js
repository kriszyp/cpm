...
function writeCurrentPackage(){
	...
	fs.write(targetPackage + "/packages.js", "require(" + 
			...
			");\ndefine('packages',[],{load: function(id, parentRequire, onLoad){parentRequire([id], onLoad);}});"); 
}
...
function installPackage(url, version, location){
	...
	if(!needsUpgrade(url)){
		print(url + " already installed");
		return;
	}
	print("Installing " + url + (relocate ? "" : " into " + location) + (version ? " version " + version : ""));
...
/*			if(installedPackageData.main){
				...
				if(main.match(/[^\.]\//)){
					...
					main = main.replace(/lib\//, packageName + '/');
					fs.write(targetLib + "/" + packageName + ".js",'define(["' +main + '"], function(main){return main;});');
				}
				else{
					// not from the lib directory, just copy it
					copy(downloadPackagePath + installedPackageData.main + ".js", targetLib + "/" + packageName + ".js");
				} 
			}
/*			["lib", "doc", "test", "bin"].forEach(function(type){
	...
	function needsUpgrade(url, verbose){
		...
		if(existing){
			...
			try{
				var exists = fs.statSync(targetPackage + "/packages/" + location).mtime.getTime();
			}catch(e){
			}
			if(exists){
				if(!version){
...
function getUri(uri, tries, returnInputStream){
	...
	return request({url:uri, headers: {"user-agent": "CommonJS Package Manager 0.1"}, encoding:"binary"}).then(function(response){
