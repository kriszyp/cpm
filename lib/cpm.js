...
print("read package " + readPackage("."));
...
function scoreVersion(version){
	...
	var parts = version.match(/(^[0-9])(\.[0-9]+)?(\.[0-9]+)?/);
	if(!parts){
		return -1;
	}
	var num = Number(parts[1]);
	if(parts[2]){
		num += Number(parts[2])/10000;
	}
	if(parts[3]){
		num += Number(parts[3])/100000000;
	}
	return num;
} 
function installPackage(url, version, location){
			...
			if(version){
				...
				for(version in versions){
					...
					if(versions[version].dist &&
							(scoreVersion(version) >= scoreVersion(bestVersion))){
						bestVersion = version;
						dist = versions[version].dist.zip;
					}
				}
	...
	when(url, function(url){
		...
		when(downloadAndUnzipArchive(url, downloadPackagePath), function(){
			...
			if(installedPackageData.main){
				...
				fs.write(targetLib + "/" + installedPackageData.name + ".js", 
					fs.read(downloadPackagePath + installedPackageData.main + ".js"));  
			}
	...
	//			print("got package");
			...
			print("mappings"+JSON.stringify(installedPackageData));
			var toInstall = [];
	...
	function needsUpgrade(url){
		...
		if(existing){
			...
			print("exists " + exists + " " + version);
			if(exists){
				if(!version){
					// compatible package
					return print(location + " already installed");				
				}
				if(version == "current"){
					print("Upgrading " + location);
					return true;
				}
				...
				if((semVerUrl && semVerExisting && semVerUrl[1] == semVerExisting[1] && 
						...
						existing == url){
					// compatible package
					return print("Compatible version " + existing + " already installed in " + location);
				}
					...
					return print("Incompatible package " + existing + " installed in " + location + " not compatible with " + url);
				}
			}
		} 
		return true;
